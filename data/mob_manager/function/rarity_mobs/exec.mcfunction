...
$execute unless predicate {"type":"minecraft:int_value_check","value":{"type":"minecraft:storage","storage":"eden:settings","path":"mob_manager.rarity_mobs.$(type).spawnchance"},"test":{"min":0.01}} run return run tag @s add mob_manager.rarity_mobs.not_$(type)
$execute unless predicate {"type":"minecraft:random_chance","chance":$(spawnchance)} run return run tag @s add mob_manager.rarity_mobs.not_$(type)
