$execute store result storage eden:temp mob_settings.camel.scale float 0.01 run random value $(scale_min)..$(scale_max)
$execute store result storage eden:temp mob_settings.camel.health float $(health) run attribute @s minecraft:max_health base get
$execute store result storage eden:temp mob_settings.camel.tempt_range float $(tempt_range) run attribute @s minecraft:tempt_range base get
$execute store result storage eden:temp mob_settings.camel.follow_range float $(follow_range) run attribute @s minecraft:follow_range base get
