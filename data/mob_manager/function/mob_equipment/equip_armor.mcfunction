...
$execute unless predicate {"condition":"minecraft:random_chance","chance":$(enchantment)} run item modify entity @s armor.$(slot) eden:enchant_with_levels
$execute if data storage eden:settings mob_manager.mob_equipment.$(slot){rngdmg:"enabled"} run item modify entity @s armor.$(slot) eden:random_damage
$execute if data storage eden:settings mob_manager.mob_equipment.misc{leathercolor:"enabled"} if items entity @s armor.$(slot) #eden:leather_armor run data modify entity @s equipment.$(slot).components."minecraft:dyed_color" set value $(leathercolor)
