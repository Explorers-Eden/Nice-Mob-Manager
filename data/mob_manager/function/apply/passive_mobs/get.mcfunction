...
$execute if data storage eden:settings mob_manager.$(type){breed:"disabled"} run tag @s add eden.settings.breed.disabled
$execute if data storage eden:settings mob_manager.$(type){drown:"disabled"} run tag @s add eden.settings.drown.disabled
...
execute if data storage eden:settings mob_manager.misc{skeletonhorsetrap:"disabled"} as @s[type=minecraft:skeleton_horse] run data modify entity @s SkeletonTrap set value 0b

function mob_manager:apply/passive_mobs/exec with storage eden:temp mob_manager